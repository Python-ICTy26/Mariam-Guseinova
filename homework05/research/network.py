...
def ego_network(user_id: tp.Optional[int] = None, friends=None) -> tp.List[tp.Tuple[int, int]]:
    """
    ...
    answer = reduce(
        lambda x, y: x + y, [[(el["id"], i) for i in el["common_friends"]] for el in mutual_friends]
    )
    return answer
